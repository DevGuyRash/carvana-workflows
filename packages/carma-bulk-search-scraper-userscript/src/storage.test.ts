...
describe('storage state v2', () => {
  it('migrates legacy v1 options into v2 state', () => {
    localStorage.clear();
    localStorage.setItem(LS_OPTIONS_V1_KEY, JSON.stringify({
    ...
    const persisted = localStorage.getItem(LS_STATE_V2_KEY);
    expect(persisted).toBeTruthy();
  });

  it('merges partial v2 state with defaults', () => {
    localStorage.clear();
    localStorage.setItem(LS_STATE_V2_KEY, JSON.stringify({
      version: 2,
