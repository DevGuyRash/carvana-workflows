...
background:#1e1e2e;border:1px solid #3b82f680;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.45),0 0 0 1px rgba(59,130,246,.15);
...
background:#0f172a;color:#94a3b8;font-size:11px;font-weight:600;cursor:pointer;transition:.15s;white-space:nowrap}
#cv-jql-builder-panel .cv-jql-preset:hover{border-color:#3b82f6;color:#e2e8f0;background:rgba(59,130,246,.1)}
#cv-jql-builder-panel .cv-jql-textarea{width:100%;min-height:80px;max-height:180px;resize:vertical;
background:#0f172a;border:1px solid #334155;border-radius:8px;color:#e2e8f0;font-family:'JetBrains Mono',monospace;
font-size:12px;padding:10px;outline:none;transition:.15s}
...
#cv-jql-builder-panel .cv-jql-btn-secondary{background:#1e293b;color:#94a3b8;border:1px solid #334155}
#cv-jql-builder-panel .cv-jql-btn-secondary:hover{border-color:#3b82f6;color:#e2e8f0}
"#;

const PRESETS: &[(&str, &str)] = &[
    ("My Open", "assignee = currentUser() AND resolution = Unresolved ORDER BY updated DESC"),
    ("Unresolved", "resolution = Unresolved ORDER BY created DESC"),
    ("Recent Updated", "updated >= -7d ORDER BY updated DESC"),
    ("AP Queue", "project = AP AND resolution = Unresolved ORDER BY priority DESC, created ASC"),
];
...
fn build_panel_html() -> String {
  ...
  <span class="cv-jql-title">⚡ JQL Builder</span>
...
<div class="cv-jql-body">
  ...
  <div class="cv-jql-actions">
    ...
    <button class="cv-jql-btn cv-jql-btn-primary" data-cv-action="search">▶ Search</button>
  </div>
</div>"#
    )
}
...
fn get_jql_for_preset(label: &str) -> Option<&'static str> {
    PRESETS.iter().find(|(l, _)| *l == label).map(|(_, jql)| *jql)
}

fn toggle_panel_visibility() -> Result<(), WasmRuntimeError> {
    let doc = document()?;
    if let Some(panel) = doc.get_element_by_id(PANEL_ID) {
        let html = panel
            .dyn_into::<web_sys::HtmlElement>()
            .map_err(|_| WasmRuntimeError::from("panel cast failed"))?;
        let display = html.style().get_property_value("display").unwrap_or_default();
        if display == "none" {
...
fn create_panel() -> Result<(), WasmRuntimeError> {
    ...
    let preset_handler = Closure::<dyn FnMut(web_sys::Event)>::wrap(Box::new(move |e: web_sys::Event| {
        let Some(target) = e.target() else { return };
        let Ok(el) = target.dyn_into::<web_sys::Element>() else { return };
        let Some(label) = el.get_attribute("data-cv-jql-preset") else { return };
        let Some(jql) = get_jql_for_preset(&label) else { return };
    ...
    let action_handler = Closure::<dyn FnMut(web_sys::Event)>::wrap(Box::new(move |e: web_sys::Event| {
        let Some(target) = e.target() else { return };
        let Ok(el) = target.dyn_into::<web_sys::Element>() else { return };
        let Some(action) = el.get_attribute("data-cv-action") else { return };
        ...
        match action.as_str() {
            "apply" => { let _ = apply_jql(&trimmed); }
            "search" => { let _ = apply_and_search(&trimmed); }
            _ => {}
        }
    }));
...
pub fn install(doc: &web_sys::Document) -> Result<Value, WasmRuntimeError> {
    ...
    let body = doc.body().ok_or_else(|| WasmRuntimeError::from("body unavailable"))?;

    ensure_advanced_mode(doc);
    let button_inserted = ensure_builder_button(doc)?;

    if body.get_attribute("data-cv-jql-builder-installed").is_none() {
        body.set_attribute("data-cv-jql-builder-installed", "true").ok();
    }
    ...
    if !Reflect::has(window.as_ref(), &JsValue::from_str(OBSERVER_KEY)).unwrap_or(false) {
        ...
        Reflect::set(window.as_ref(), &JsValue::from_str(OBSERVER_KEY), observer.as_ref()).ok();
        cb.forget();
    }
