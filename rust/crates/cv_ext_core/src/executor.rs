...
impl ActionExecutor for NoopExecutor {
    ...
    fn wait_for(&mut self, _selector: &str, _timeout_ms: u32) -> Result<Value, String> {
        unsupported("wait_for")
    }

    fn click(&mut self, _selector: &str) -> Result<Value, String> {
        unsupported("click")
    }

    fn type_text(&mut self, _selector: &str, _text: &str) -> Result<Value, String> {
        unsupported("type")
    }

    fn extract_table(&mut self, _selector: &str) -> Result<Value, String> {
        unsupported("extract_table")
    }

    fn execute_command(&mut self, _command: &str) -> Result<Value, String> {
        unsupported("execute")
    }
}
